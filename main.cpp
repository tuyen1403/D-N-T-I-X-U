...
class MultiRunEngine {
    ...
    static double calcMedian(vector<double> v) {
        ...
        int n = (int)v.size();
        if (n == 0) return 0;
        return n % 2 == 0 ? (v[n/2-1] + v[n/2]) / 2.0 : v[n/2];//Trường hợp số lượng phần tử lẻ (n % 2 != 0):Trung vị chính là phần tử nằm ở vị trí chính giữa: v[n/2]; trường hợp là số chẵn (v[n/2 - 1] + v[n/2]) / 2.0.
    }
